...
class ComponentParameters
{
     ...
     * @param array $prepareParams - Array with settings for prepare parameters of merged the component.
     *        For example:
     *          <code>["SELECT_FIELDS" => ["RENAME" => 'LIST_SELECT_FIELDS', "MOVE" => 'LIST']</code>
     *          Options:
     *          <ul>
    ...
    public static function getParameters($component, $prepareParams = [], $arCurrentValues, $selectOnlyListed = false)
    {
        ...
        if (!empty($prepareParams)) {
            foreach ($componentParams["PARAMETERS"] as $code => &$params) {
                if (
                    $prepareParams[$code]["DELETE"]
                    || ($selectOnlyListed === true && !isset($prepareParams[$code]))
                ) {
            ...
            $componentParams["PARAMETERS"] = array_replace_recursive(
